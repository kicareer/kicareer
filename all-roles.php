    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="row">
                    <?php
                    $fetch_role = $conn->prepare("SELECT role, COUNT(*) as job_count FROM post WHERE role<>'' GROUP BY role ORDER BY count(sno) ASC");
                    $fetch_role->execute();
                    foreach ($fetch_role->fetchAll(PDO::FETCH_ASSOC) as $role) {
                        ?>
                        <div class="col-lg-4 mb-3">
                            <a href="./?filter_by_role&role=<?=$role['role']?>" class="btn btn-light btn-block text-left">
                                <?=$role['role']?> 
                                <!-- <span class="badge badge-primary float-right"><?=$role['job_count']?></span> -->
                            </a>
                        </div>
                        <?php
                    }
